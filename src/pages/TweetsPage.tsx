  ...
  const onClickComment = async () => {
    if (like + (!isLike ? -1 : 1)) {
      await axios.get(`comment/${_id}`).then(async (response) => {
        ...
        await axios
          .put(`/likeComment/${idComentario}/${owner}`)
          .then(async (response) => {
    ...
    <SafeAreaView>
      ...
      >
        ...
        >
          ...
          <View style={{paddingLeft: 50, paddingTop: 15,}}>
          ...
          <View style={{ paddingHorizontal: 25, paddingTop: 5 }}></View>

          <TextInput
            color="#066cb4"
            placeholder="Agregar comentario al Tweet"
            onChangeText={(text) => setText(text)}
            style={{ paddingHorizontal: 20, paddingVertical: 30, paddingTop: 5 }}
            value={comment}
            numberOfLines={4}
            maxLength={120}
            editable
